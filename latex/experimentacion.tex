...
La complejidad teorica calculado para nuestras implementaciones de la Eliminación Gaussiana y LU es de: 

Consideraciones:
\begin{itemize}
    \item Los casos de tests para esta experimentación se generaron a partir de los valores del Alto Horno de Hierro, vease Sección \ref{instancias}, variando la cantidad de puntos de la discretización y seteando el $ninst$ en 1.
    \item El tamaño de la matriz generada, a partir de la entrada, varia de $9x9$ hasta $180x180$.
    \item Los tiempos de ejecución se midieron con la biblioteca chrono y estos fueron convertidos a nanosegundos.
    \item Para cada caso de test, se midió la ejecución de los metodos Eliminación Gaussiana y LU.
    \item Para cada tamaño de la matriz$n(m+1)$ se promediaron los resultados obtenidos para cada método.
\end{itemize}
...
\begin{center}
    ...
    \begin{axis}[
        title={},
        xlabel={Tamanio de la matriz(n*(m+1))},
...
\begin{center}
    ...
    \begin{axis}[
        title={},
        xlabel={Tamanio de la matriz($n*(m+1)$)},
        ylabel={Tiempos de ejecucion (nanoseconds) / $(n*(m+1)^2)$},
        scaled x ticks=false,
        scaled y ticks=false,
        enlargelimits=0.05,
        width=0.5\textwidth,
...
\begin{center}
    ...
    \begin{axis}[
        title={},
        xlabel={Tamanio de la matriz($n*(m+1)$)},
        ylabel={Tiempos de ejecucion (nanoseconds) / $(n*(m+1)^3)$},
        scaled x ticks=false,
        scaled y ticks=false,
        enlargelimits=0.05,
        width=0.5\textwidth,
